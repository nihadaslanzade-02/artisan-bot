...
def verify_artisan_id_card(artisan_id, is_verified, admin_id):
    """Verify or reject artisan's ID card
    
    Args:
        artisan_id (int): ID of the artisan
        is_verified (bool): Whether to verify (True) or reject (False)
        admin_id (int): ID of the admin performing action
    ...
    try:
        ...
        # Get current artisan info
        cursor.execute(
            "SELECT name, id_verification_status FROM artisans WHERE id = %s",
            (artisan_id,)
        )
        artisan_info = cursor.fetchone()
        
        if not artisan_info:
            logger.error(f"Artisan {artisan_id} not found")
            return False
        
        old_status = artisan_info[1]
        
        if is_verified:
            # Verify ID card - set as verified and active
            cursor.execute(
                """
                UPDATE artisans 
            ...
            # Notify artisan about verification
            cursor.execute(
                "SELECT telegram_id FROM artisans WHERE id = %s",
                (artisan_id,)
            )
            ...
            if telegram_result and telegram_result[0]:
                ...
                decrypted_telegram_id = decrypt_data(telegram_result[0])
                ...
                asyncio.create_task(bot.send_message(
                    chat_id=int(decrypted_telegram_id),
                    text="✅ *Şəxsiyyət vəsiqəniz təsdiqləndi!*\n\n"
                         "Hesabınız aktiv edildi və artıq sifarişlər qəbul edə bilərsiniz.",
                    parse_mode="Markdown"
                ))
        else:
            # Reject ID card - set as rejected, keep inactive
            cursor.execute(
                """
                UPDATE artisans 
            ...
            if telegram_result and telegram_result[0]:
                ...
                asyncio.create_task(bot.send_message(
                    chat_id=int(decrypted_telegram_id),
                    text="❌ *Şəxsiyyət vəsiqəniz rədd edildi*\n\n"
                         "Şəxsiyyət vəsiqəniz aydın görünmədiyi üçün rədd edildi. "
                         "Zəhmət olmasa, daha aydın şəkil göndərərək yenidən qeydiyyatdan keçin.",
                    parse_mode="Markdown"
                ))
        ...
        cursor.execute(
            ...
            (admin_id, action_type, target_id, target_type, old_value, new_value, details)
            VALUES (%s, 'id_verification', %s, 'artisan', %s, %s, %s)
            """,
            (admin_id, artisan_id, old_status, new_status, 
             f"ID verification {'approved' if is_verified else 'rejected'}")
        )
        
        conn.commit()
        logger.info(f"Artisan {artisan_id} ID verification {'approved' if is_verified else 'rejected'} by admin {admin_id}")
        return True
        
    except Exception as e:
        logger.error(f"Error verifying artisan ID card: {e}")
        if conn:
            conn.rollback()
        return False
    finally:
...
def get_pending_id_verifications():
    """Get all artisans with pending ID verification
    
    Returns:
        list: List of artisans pending verification
    """
    try:
        query = """
            SELECT id, name, phone, city, service, id_card_image_id, created_at
            FROM artisans 
            WHERE id_verification_status = 'pending'
            ORDER BY created_at ASC
        """
        
        result = execute_query(query, fetchall=True, dict_cursor=True)
        return result if result else []
        
    except Exception as e:
        logger.error(f"Error getting pending ID verifications: {e}")
        return []
